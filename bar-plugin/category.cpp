...
Category::Category(PojkMenuDirectory* directory) :
	m_button(NULL),
	m_model(NULL),
	...
	m_has_subcategories(false)
{
	...
	if (directory)
	{
		icon = pojk_menu_directory_get_icon_name(directory);
		text = pojk_menu_directory_get_name(directory);
		tooltip = pojk_menu_directory_get_comment(directory);
	}
	else
	{
...
Category* Category::append_menu(PojkMenuDirectory* directory)
{
