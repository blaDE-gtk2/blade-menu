...
static void blademenu_config_dialog_delete(ConfigurationDialog* dialog)
{
	delete dialog;
}
	...
	m_plugin(plugin)
{
	...
	m_window = xfce_titled_dialog_new_with_buttons(_("blademenu"),
			window,
	...
	g_signal_connect_swapped(m_window, "destroy", G_CALLBACK(blademenu_config_dialog_delete), this);
...
{
	GtkWidget* chooser = blxo_icon_chooser_dialog_new(_("Select An Icon"),
			GTK_WINDOW(m_window),
	...
	blxo_icon_chooser_dialog_set_icon(BLXO_ICON_CHOOSER_DIALOG(chooser), m_plugin->get_button_icon_name().c_str());

	if (gtk_dialog_run(GTK_DIALOG (chooser)) == GTK_RESPONSE_ACCEPT)
	{
		gchar* icon = blxo_icon_chooser_dialog_get_icon(BLXO_ICON_CHOOSER_DIALOG(chooser));
		blade_bar_image_set_from_source(BLADE_BAR_IMAGE(m_icon), icon);
...
void ConfigurationDialog::response(GtkDialog*, int response_id)
{
	...
	{
		bool result = g_spawn_command_line_async("blxo-open --launch WebBrowser " PLUGIN_WEBSITE, NULL);
...
GtkWidget* ConfigurationDialog::init_appearance_tab()
{
	...
	// Create bar button section
	GtkTable* bar_table = GTK_TABLE(gtk_table_new(4, 2, false));
	gtk_table_set_col_spacings(bar_table, 12);
	gtk_table_set_row_spacings(bar_table, 6);

	GtkWidget* bar_frame = xfce_gtk_frame_box_new_with_content(_("Bar Button"), GTK_WIDGET(bar_table));
	gtk_box_pack_start(contents_vbox, bar_frame, false, false, 6);
	...
	gtk_table_attach(bar_table, label, 0, 1, 0, 1, GTK_FILL, GtkAttachOptions(GTK_EXPAND | GTK_FILL), 0, 0);
	...
	gtk_table_attach_defaults(bar_table, m_button_style, 1, 2, 0, 1);
	...
	gtk_misc_set_alignment(GTK_MISC(label), 0, 0.5);
	gtk_table_attach(bar_table, label, 0, 1, 1, 2, GTK_FILL, GtkAttachOptions(GTK_EXPAND | GTK_FILL), 0, 0);

	m_title = gtk_entry_new();
	gtk_entry_set_text(GTK_ENTRY(m_title), m_plugin->get_button_title().c_str());
	gtk_table_attach_defaults(bar_table, m_title, 1, 2, 1, 2);
	...
	gtk_misc_set_alignment(GTK_MISC(label), 0, 0.5);
	gtk_table_attach(bar_table, label, 0, 1, 2, 3, GTK_FILL, GtkAttachOptions(GTK_EXPAND | GTK_FILL), 0, 0);
	...
	gtk_table_attach_defaults(bar_table, icon_alignment, 1, 2, 2, 3);

	m_icon = blade_bar_image_new_from_source(m_plugin->get_button_icon_name().c_str());
	blade_bar_image_set_size(BLADE_BAR_IMAGE(m_icon), 48);
	gtk_container_add(GTK_CONTAINER(m_icon_button), m_icon);

	m_button_single_row = gtk_check_button_new_with_mnemonic(_("Use a single _bar row"));
	gtk_table_attach_defaults(bar_table, m_button_single_row, 1, 2, 3, 4);
...
GtkWidget* ConfigurationDialog::init_behavior_tab()
{
	...
	m_position_search_alternate = gtk_check_button_new_with_mnemonic(_("Position _search entry next to bar button"));
	...
	m_position_categories_alternate = gtk_check_button_new_with_mnemonic(_("Position cate_gories next to bar button"));
