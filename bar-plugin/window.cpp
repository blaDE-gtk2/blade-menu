	...
	m_supports_alpha(false)
{
	...
	gtk_widget_set_name(GTK_WIDGET(m_window), "blademenu-window");
	// Untranslated window title to allow window managers to identify it; not visible to users.
	gtk_window_set_title(m_window, "blademenu");
	...
	m_bars_box = GTK_BOX(gtk_vbox_new(false, 0));
	gtk_box_pack_start(m_contents_box, GTK_WIDGET(m_bars_box), true, true, 0);
	gtk_box_pack_start(m_bars_box, m_favorites->get_widget(), true, true, 0);
	gtk_box_pack_start(m_bars_box, m_recent->get_widget(), true, true, 0);
	gtk_box_pack_start(m_bars_box, m_applications->get_widget(), true, true, 0);
...
void BladeMenu::Window::show(GtkWidget* parent, bool horizontal)
{
	...
	if (parent != NULL)
	{
		// Wait up to half a second for auto-hidden bars to be shown
	...
	{
		gtk_box_reorder_child(m_contents_box, GTK_WIDGET(m_bars_box), 1);
		gtk_box_reorder_child(m_contents_box, GTK_WIDGET(m_sidebar), 2);
	}
	else
	{
		gtk_box_reorder_child(m_contents_box, GTK_WIDGET(m_bars_box), 2);
...
void BladeMenu::Window::on_context_menu_destroyed()
{
...
void BladeMenu::Window::set_categories(const std::vector<SectionButton*>& categories)
{
...
void BladeMenu::Window::set_items()
{
...
void BladeMenu::Window::unset_items()
{
...
gboolean BladeMenu::Window::on_enter_notify_event(GtkWidget*, GdkEvent* event)
{
...
gboolean BladeMenu::Window::on_leave_notify_event(GtkWidget*, GdkEvent* event)
{
	GdkEventCrossing* crossing_event = reinterpret_cast<GdkEventCrossing*>(event);
	if ((crossing_event->detail == GDK_NOTIFY_INFERIOR)
...
gboolean BladeMenu::Window::on_button_press_event(GtkWidget*, GdkEvent* event)
{
...
gboolean BladeMenu::Window::on_key_press_event(GtkWidget* widget, GdkEvent* event)
{
	GdkEventKey* key_event = reinterpret_cast<GdkEventKey*>(event);

	// Hide if escape is pressed and there is no text in search entry
	if ( (key_event->keyval == GDK_Escape) && blxo_str_is_empty(gtk_entry_get_text(m_search_entry)) )
	{
		hide();
		return true;
	}
...
gboolean BladeMenu::Window::on_key_press_event_after(GtkWidget* widget, GdkEvent* event)
{
...
gboolean BladeMenu::Window::on_map_event(GtkWidget*, GdkEvent*)
{
...
gboolean BladeMenu::Window::on_configure_event(GtkWidget*, GdkEvent* event)
{
...
void BladeMenu::Window::on_screen_changed_event(GtkWidget* widget, GdkScreen*)
{
...
gboolean BladeMenu::Window::on_expose_event(GtkWidget* widget, GdkEventExpose*)
{
...
void BladeMenu::Window::favorites_toggled()
{
	m_favorites->reset_selection();
	gtk_widget_hide(m_recent->get_widget());
	gtk_widget_hide(m_applications->get_widget());
...
void BladeMenu::Window::recent_toggled()
{
...
void BladeMenu::Window::category_toggled()
{
...
void BladeMenu::Window::show_favorites()
{
...
void BladeMenu::Window::show_default_page()
{
	// Switch to favorites bar
	m_default_button->set_active(true);
...
void BladeMenu::Window::search()
{
	...
	if (visible)
	{
		...
		gtk_widget_hide(GTK_WIDGET(m_bars_box));
	...
	else
	{
		// Show active bar
		gtk_widget_hide(m_search_results->get_widget());
		gtk_widget_show(GTK_WIDGET(m_bars_box));
