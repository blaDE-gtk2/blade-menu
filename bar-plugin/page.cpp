...
void Page::create_context_menu(GtkTreeIter* iter, GdkEvent* event)
{
	...
	menuitem = gtk_menu_item_new_with_label(_("Add to Bar"));
	g_signal_connect_slot<GtkMenuItem*>(menuitem, "activate", &Page::add_selected_to_bar, this);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
...
void Page::add_selected_to_bar()
{
	// Connect to Xfce bar through D-Bus
	GError* error = NULL;
	...
	if (proxy)
	{
		...
		if (!g_dbus_proxy_call_sync(proxy,
				...
				&error))
		{
			xfce_dialog_show_error(NULL, error, _("Unable to add launcher to bar."));
			g_error_free(error);
		}
...
void Page::edit_selected()
{
	...
	gchar* command = g_strconcat("blxo-desktop-item-edit ", uri, NULL);
	g_free(uri);
