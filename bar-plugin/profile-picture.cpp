...
void ProfilePicture::on_file_changed(GFileMonitor*, GFile* file, GFile*, GFileMonitorEvent)
{
	...
	BladeBarImage* image = BLADE_BAR_IMAGE(m_image);
	if (face)
	{
		blade_bar_image_set_size(image, -1);
		blade_bar_image_set_from_pixbuf(image, face);
		g_object_unref(face);
	}
	else
	{
		blade_bar_image_set_size(image, height);
		blade_bar_image_set_from_source(image, "avatar-default");
	}
}
