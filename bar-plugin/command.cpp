...
enum
{
	BLADEMENU_COMMAND_UNCHECKED = -1,
	BLADEMENU_COMMAND_INVALID,
	BLADEMENU_COMMAND_VALID
};
...
Command::Command(const gchar* icon, const gchar* text, const gchar* command, const gchar* error_text) :
	...
	m_status(BLADEMENU_COMMAND_UNCHECKED),
	m_shown(true)
{
	check();
}
...
GtkWidget* Command::get_button()
{
	...
	gtk_widget_set_sensitive(m_button, m_status == BLADEMENU_COMMAND_VALID);
...
GtkWidget* Command::get_menuitem()
{
	...
	gtk_widget_set_sensitive(m_menuitem, m_status == BLADEMENU_COMMAND_VALID);
...
void Command::check()
{
	if (m_status == BLADEMENU_COMMAND_UNCHECKED)
	{
		gchar** argv;
		if (g_shell_parse_argv(m_command, NULL, &argv, NULL))
		{
			gchar* path = g_find_program_in_path(argv[0]);
			m_status = path ? BLADEMENU_COMMAND_VALID : BLADEMENU_COMMAND_INVALID;
			g_free(path);
			g_strfreev(argv);
		}
		else
		{
			m_status = BLADEMENU_COMMAND_INVALID;
		}
	}
